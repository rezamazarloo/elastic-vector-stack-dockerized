...
until curl -s -u "$ES_USER:$ES_PASS" "$ES_URL/_cluster/health" | grep -vq '"status":"red"'; do
  sleep 5
done
...
for role_file in /roles/*.json; do
  ...
  curl -s -u "$ES_USER:$ES_PASS" -X PUT "$ES_URL/_security/role/$role_name" \
...
curl -s -u "$ES_USER:$ES_PASS" -X POST "$ES_URL/_security/user/vector" \
     -H "Content-Type: application/json" \
     -d "{
...
curl -s -u "$ES_USER:$ES_PASS" -X POST "$ES_URL/_security/user/backend" \
     -H "Content-Type: application/json" \
     -d "{
